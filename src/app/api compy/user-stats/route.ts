...
export async function GET() {
  try {
    // Query for users who started today
    ...
    // Query for users started in the last 7 days
    ...
    return NextResponse.json({
      success: true,
      data: {
        users_started_today: Number(usersTodayResult[0]?.users_started_today ?? 0),
        total_users: Number(totalUsersResult[0]?.total_users ?? 0),
        users_started_last_7_days: Number(usersLast7DaysResult[0]?.users_started_last_7_days ?? 0),
        active_users: Number(activeUsersResult[0]?.active_users ?? 0),
      },
    });
  } catch (error: any) {
